...
class Callbacks:
    def __init__(self, client):
        ...
        self.chat_actions_start = {"Typing": self.on_user_typing_start}
        self.chat_actions_end = {"Typing": self.on_user_typing_end}
    def _resolve_chat_message(self, data):
        key = f"{data['o']['chatMessage']['type']}:{data['o']['chatMessage'].get('mediaType', 0)}"
        handler = self.chat_methods.get(key, self.default)
        return handler(data)
    def _resolve_chat_action_start(self, data):
        key = data['o'].get('actions', 0)
        handler = self.chat_actions_start.get(key, self.default)
        return handler(data)
    def _resolve_chat_action_end(self, data):
        key = data['o'].get('actions', 0)
        handler = self.chat_actions_end.get(key, self.default)
        return handler(data)
    def resolve(self, data):
        data = json.loads(data)
        return self.methods.get(data.get("t"), self.default)(data)
    ...
    def on_text_message(self, data):
        self.call(inspect.currentframe().f_code.co_name, objects.Event(data["o"]))
    ...
    def default(self, data):
        self.call(inspect.currentframe().f_code.co_name, data)
